...
function calcDistance(reindeer: Reindeer, time: number): number {
    const fullSprint = reindeer.speed * reindeer.sprintDuration;
    const cycleTime = reindeer.sprintDuration + reindeer.restDuration
    const fullCycles = Math.floor(time / cycleTime);
    const remainder = time % cycleTime;
    if (remainder >= reindeer.sprintDuration) {
        return (fullCycles + 1) * fullSprint;
    }
    return fullCycles * fullSprint + remainder * reindeer.speed;
}

function solution(lines: Array<string>): RESULT_TYPE {
    const TIME = 2503;
    return Math.max(...lines.map(parse)
        .map(reindeer => calcDistance(reindeer, TIME)))
}

const TEST_RESULTS: Array<RESULT_TYPE> = [
    2660 // No actual example given, but... here's what I got when I ran the test.
];
