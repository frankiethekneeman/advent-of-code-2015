...
function forceCornersOn(state:Life): Life {
    const maxX = state.length - 1;
    return state.map((row, x) => {
        if (x > 0 && x < maxX) {
            return row;
        }
        const maxY = row.length - 1;
...
function solution(lines: Array<string>): RESULT_TYPE {
    let state = forceCornersOn(parse(lines));
    for (let i = 0; i < STEPS; i++) {
        state = forceCornersOn(increment(state))
    }
