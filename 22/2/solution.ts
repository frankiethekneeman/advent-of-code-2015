...
function simulateRound(preRound: Combat, spell?: Spell): Combat {
    ...
    const hardMode = {...preRound, player: {...preRound.player, hp: preRound.player.hp - 1}}
    if (isComplete(hardMode)) return hardMode;
    const preSpell = resolveEffects({...hardMode, player: {...hardMode.player, defense: 0}} );
