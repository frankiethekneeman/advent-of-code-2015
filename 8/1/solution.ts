...
ESCAPE_SEQUENCE_COSTS.set('\\"', 1);
ESCAPE_SEQUENCE_COSTS.set('\\\\', 1);
ESCAPE_SEQUENCE_COSTS.set('\\x', 3);


function calculateMemory(str: string): number {
    let memory = str.length - 2;
...
function solution(lines: Array<string>): RESULT_TYPE {
    ...
    const memoryLength = strings.map(calculateMemory).reduce(sum);
