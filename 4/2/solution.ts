...
function hash(key: string, data: number): string {
    const hasher = cryptography.createHash('md5')
    hasher.update(key + data);
    return hasher.digest('hex');
}

function solution(lines: Array<string>): RESULT_TYPE {
    const secretKey = lines.join('');
    let candidate = 1;
    while(!hash(secretKey, candidate).startsWith('000000')) {
        candidate++;
    }
    return candidate;
}
