...
const NICE_MARKERS: Array<RegExp> = [
    /[aeiou].*[aeiou].*[aeiou]/, // Three Vowels
    /(.)\1/ // Double Letter
]

const NAUGHTY_MARKERS: Array<RegExp> = [
    /ab|cd|pq|xy/ // Forbidden substrings
]

function solution(lines: Array<string>): RESULT_TYPE {
    return lines.filter(l => l.length > 0)
        .filter(line => NICE_MARKERS.every(marker => marker.test(line))) // All Nice markers
        .filter(line => !NAUGHTY_MARKERS.some(marker => marker.test(line))) // No naughty markers
        .length;
}
